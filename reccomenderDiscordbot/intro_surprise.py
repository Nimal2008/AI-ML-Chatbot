...
# Step 7: Simple function to get recommendations for a user
...
# Step 8: Simple test - hide a movie and compare prediction
...
if users:
    ...
    # Find a movie this user rated
    user_row = df[df['Name'] == test_user].iloc[0]
    rated_movies = [movie for movie in df.columns[1:] if pd.notna(user_row[movie])]

    if rated_movies:
        movie_to_hide = rated_movies[0]
        original_rating = user_row[movie_to_hide]

        # Predict rating for this movie
        pred = model.predict(test_user, movie_to_hide)
        predicted_rating = pred.est

        print(f"Movie: {movie_to_hide}")
        print(f"Original rating: {original_rating}")
        print(f"Predicted rating: {predicted_rating:.2f}")
        print(f"Error: {abs(original_rating - predicted_rating):.2f}")

    # Get recommendations
    get_recommendations(test_user, 3)
